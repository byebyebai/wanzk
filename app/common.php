...
function get_picture_url($id = 0)
{
    ...
    if (!empty($info['path'])) {
        if($id<=194){
            ...
            return $root_url .'upload/images/'.$info['path'];
            
        }else{
        	
            return $root_url . 'images'.$info['path'];
...
/**
 * [getPid description] 获取类别的父级id
 * @param  [type] $id [description] 
 * @return [type]     [description]
 */
function getPid($id,$list = []){
    ...
    if($info){
        array_push($list_arr, $info);
        getPid($info['pid'],$list_arr);
          
        if($info['pid'] == 0){
            //print_r($list_arr);
            return $list_arr;    
            // var_dump($list);exit;
            // print_r(getPid($info['pid'],$list,$k));exit;
...
/**
 * [getArctile description] 获取文章根据排序条数
 * @param  array   $where [description]
 * @param  string  $order [description]
 * @param  integer $limit [description]
 * @return [type]         [description]
 */
function getArctile($where = [],$order = '',$field = '*',$limit = 0){
    return Db::name('article')->where($where)->order($order)->field($field)->limit(0,$limit)->select();
}
/**
 * [getCategory description]获取分类
 * @param  array   $where [description]
 * @param  string  $field [description]
 * @param  integer $limit [description]
 * @return [type]         [description]
 */
function getCategory($where = [],$field = '*',$limit = 1)
{
    return Db::name('article_category')->where($where)->field($field)->limit($limit)->select();
}
/**
 * [getDoman description] 域名获取器
 * @return [type] [description]
 */
function getDoman(){
 ...
 * @return array 
 */
function random_data($num,$table,$where=[])
{
...
/**
 * [getConfig description] 获取配置的数据 通过name
...
//     $data['day'] = floor((strtotime(getConfig('test_time'))-time())/3600/24);
//     return $data;
// }
//注册截止日期
...
/**
 * [ArticleTag description]添加tag标签
 * @param [type]  $tag_name [description]
 * @param integer $tag_id    [description] 为空执行添加操作并返回新增id
 */
function ArticleTag($tag_name = '',$aid = '',$tag_id = null){
    if($tag_id){
        if(!empty($tag_name)){
            $where['tag_name'] = ['like','%'.$tag_name."%"];
        } 
        $where['tag_id'] = $tag_id;
        return Db::name('tag')->where($where)->find();
    }else{

        if($tag_name){
            //通过,分开标签名称
            $tag_array = explode(',', trim($tag_name));
            foreach ($tag_array as $k => $v) {
                $where['tag_name'] = ['like','%'.$v."%"];
                $res = Db::name('tag')->where($where)->find();

                if($res){
                    $data['aid'] = $res['aid'].','.$aid;
                    $dataArr = array_unique(explode(',', trim($data['aid'])));
                    ...
                    Db::name('tag')->where($where)->update(['aid'=>$newData]);
                }else{
                    $data['tag_name'] = $v;
                    $data['aid'] = $aid;
                    Db::name('tag')->insertGetId($data);
                }               
            }
...
function time2week(){
    ...
    return date('Y年m月d日',strtotime(config('test_time'))).'星期'.$weekarray[date("w",strtotime(config('test_time')))];
}
...
/**
 * [unicode_decode description]unicode转中文
...
/**
 * [remove_duplicate description] 二维数组根据某个元素去除重复
...
/**
 * [getProvince description] 获取省级
 * @return [type] [description]
 */
function getProvince(){
    return Db::name('district')->where('level',1)->select();
}
function getCity($pid = ''){
    return Db::name('district')->where(['level'=>2,'upid'=>$pid])->select();
}
function getAddressNameById($id){

    return Db::name('district')->where('id',$id)->value('name');

}
/**
 * [cutstr_html description] 去除html标签 去除留白
...
function cutstr_html($string, $sublen){  
    ...
    $string = mb_substr(strip_tags($string),0,$sublen,"utf-8");
...
/*
 字符串时间转日期
 * */	
function str2date($str = '',$format = 'Y-m-d H:i:s'){
	if($str){
